...
namespace Nintenlord
{
    abstract class AbstractROM
    {
        ...
        public virtual string GetSjisString(int offset)
        {
            var length = 0;
            while (ROMdata[offset + length] != 0) length++;

            return Encoding.GetEncoding("shift-jis").GetString(ROMdata,offset,length);
        }
